...
describe("createAwardSchema", () => {
    ...
    it("should fail with empty name", () => {
        expect(createAwardSchema.safeParse({ ...valid, name: "" }).success).toBe(false)
    })
    ...
    it("should fail with empty deadline", () => {
        expect(createAwardSchema.safeParse({ ...valid, deadline: "" }).success).toBe(false)
    })

    it("should fail with empty description", () => {
        expect(createAwardSchema.safeParse({ ...valid, description: "" }).success).toBe(false)
    })

    it("should fail with description over 1000 characters", () => {
        const longDesc = "a".repeat(1001)
        expect(createAwardSchema.safeParse({ ...valid, description: longDesc }).success).toBe(false)
    })
})
