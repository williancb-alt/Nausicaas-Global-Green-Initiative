...
describe("ProtectedRoute", () => {
    ...
    it("should show loading state", () => {
        vi.mocked(useUser).mockReturnValue({ data: undefined, isLoading: true, isError: false } as unknown as ReturnType<typeof useUser>)
        render(<ProtectedRoute><div>Protected Content</div></ProtectedRoute>)
        expect(screen.getByText("Loading...")).toBeDefined()
    })

    it("should redirect to login if not authenticated", () => {
        vi.mocked(useUser).mockReturnValue({ data: null, isLoading: false, isError: false } as unknown as ReturnType<typeof useUser>)
        render(<ProtectedRoute><div>Protected Content</div></ProtectedRoute>)
    ...
    it("should redirect to login if there is an error", () => {
        vi.mocked(useUser).mockReturnValue({ data: undefined, isLoading: false, isError: true } as unknown as ReturnType<typeof useUser>)
        render(<ProtectedRoute><div>Protected Content</div></ProtectedRoute>)
        const nav = screen.getByTestId("navigate")
        expect(nav.getAttribute("data-to")).toBe("/login")
    })
    ...
    it("should redirect to home if admin is required but user is not admin", () => {
        vi.mocked(useUser).mockReturnValue({ data: { admin: false }, isLoading: false, isError: false } as unknown as ReturnType<typeof useUser>)
    ...
    it("should render children if authenticated and admin check passes", () => {
        vi.mocked(useUser).mockReturnValue({ data: { admin: true }, isLoading: false, isError: false } as unknown as ReturnType<typeof useUser>)
        render(<ProtectedRoute requireAdmin={true}><div>Admin Content</div></ProtectedRoute>)
