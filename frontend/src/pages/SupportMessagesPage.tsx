        ...
        <div style={{ backgroundColor: "#f8fafc", minHeight: "100vh" }} className="pb-5">
            ...
            >
                <div className="container">
                    ...
                    <p className="lead opacity-75 mt-2">Manage, respond, and track support history officially.</p>
                </div>
            </div>
            ...
            <div className="container">
                {/* Alerts */}
                {successMsg && (
                    <div className="alert alert-success border-0 shadow-sm d-flex align-items-center gap-2 mb-4" style={{ borderRadius: "12px" }}>
                ...
                {error && (
                    <div className="alert alert-danger border-0 shadow-sm d-flex align-items-center gap-2 mb-4" style={{ borderRadius: "12px" }}>
                        <AlertCircle size={20} />
                        {error}
                    </div>
                )}
                        ...
                        <div className="card border-0 shadow-sm h-100" style={{ borderRadius: "16px" }}>
                            <div className="card-body d-flex align-items-center justify-content-around py-3">
                                <div className="text-center">
                                    <div className="h4 mb-0 fw-bold text-primary">{stats.pending}</div>
                                    <small className="text-muted text-uppercase fw-semibold" style={{ fontSize: '0.65rem', letterSpacing: '0.05em' }}>New Tickets</small>
                                </div>
                                <div className="vr opacity-10"></div>
                                <div className="text-center">
                                    <div className="h4 mb-0 fw-bold text-success">{stats.replied}</div>
                                    <small className="text-muted text-uppercase fw-semibold" style={{ fontSize: '0.65rem', letterSpacing: '0.05em' }}>Total Replied</small>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        {filteredMessages.map((msg) => (
                            <div key={msg.id} className="col-12">
                                <div
                                    ...
                                    style={{
                                        borderRadius: "16px",
                                        borderLeft: msg.status === "Replied" ? "6px solid #10b981" : "6px solid #3b82f6"
                                    }}
                                >
                                            ...
                                            <div className="d-flex gap-2">
                                                {msg.status !== "Replied" ? (
                                                    <button
                                                        ...
                                                        className={`btn d-flex align-items-center gap-2 px-4 ${replyingTo === msg.id ? 'btn-light border' : 'btn-primary'}`}
                                                        style={{ borderRadius: "10px" }}
                                                    >
                                                        {replyingTo === msg.id ? <X size={18} /> : <Mail size={18} />}
                                                        {replyingTo === msg.id ? "Cancel" : "Official Reply"}
                                                    </button>
                                                ) : (
                                                    <button
                                                        onClick={() => setViewingHistory(viewingHistory === msg.id ? null : msg.id)}
                                                        className={`btn d-flex align-items-center gap-2 px-4 ${viewingHistory === msg.id ? 'btn-light border' : 'btn-outline-success border-success border-opacity-25'}`}
                                                        style={{ borderRadius: "10px" }}
                                                    >
                                                        {viewingHistory === msg.id ? <X size={18} /> : <Eye size={18} />}
                                                        {viewingHistory === msg.id ? "Hide History" : "View Sent Response"}
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="card-body bg-light bg-opacity-50 p-4">
                                        <div className="mb-4">
                                            <label className="text-muted small fw-bold text-uppercase mb-2 d-block">User Message</label>
                                            <div
                                        ...
                                        {msg.status === "Replied" && viewingHistory === msg.id && (
                                            <div className="mt-4 p-4 bg-white rounded shadow-sm border-top border-success border-4 slide-down">
                                                ...
                                                <div
                                                    ...
                                                    style={{ whiteSpace: "pre-wrap", color: "#065f46", lineHeight: "1.6", fontStyle: "italic" }}
                                                >
                                                    {msg.admin_response}
                                                </div>
                                            </div>
                                        ...
                                        {replyingTo === msg.id && (
                                            <div className="mt-4 p-4 bg-white rounded shadow-sm border-top border-primary border-4 slide-down">
                                                ...
                                                <p className="text-muted small mb-3">
                                                    This message will be sent to <strong>{msg.user.email}</strong> from the official
